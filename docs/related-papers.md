...
| 0件（欄自体が出ない） | **16/1067** |
| ビルド時間 | 27.2秒 → **38.2秒**（+11秒） |

ビルド時間の増分が単体計測（1.9秒）より大きいのは、Next.jsが複数のワーカー
プロセスで静的生成するため、索引がプロセスごとに構築されるから。
Vercel Hobbyのビルド上限には十分収まる。
...
いま論文に付いている軸は DB(10) / 研究カテゴリ(9) / 解析手法(12) / 研究デザイン(4)
だけで、**「何の病気の研究か」を表す軸が1本も無い**。研究カテゴリは
「安全性・副作用」のような研究の性格を表すもので、疾患領域ではない。
このため「循環器の研究だけ見たい」といった絞り込みができない。

OpenAlex の singleton がこの軸を無料で埋められることは確認済み。
...
| 例 | 「Cardiac, Anesthesia and Surgical Outcomes」「Cervical Cancer and HPV Research」 |

第1トピックのスコアは 0.99 / 0.89 / 0.64 / 0.19 とばらつき、第2・第3は
0.00〜0.06 でほぼノイズ。採用するなら**第1トピックのみ・スコア閾値付き**。
既存1,067件のバックフィルは0.7秒間隔で約13分、以降は週約9件。

**これは関連研究の質を上げる話ではなく、ファセット絞り込みを1軸増やす話。**
関連研究とは独立に判断してよい。
