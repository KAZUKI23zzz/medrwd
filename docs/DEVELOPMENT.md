...
### デプロイ容量とページ転送量の削減（2026-09-06）

**きっかけ**: Vercel から Deployment Storage（無料枠10GB）100%の警告。

枠は「1デプロイの大きさ×保持しているデプロイ数」。1デプロイ198MB、保持30日、
8月は33コミット（各コミットにプレビューと本番の2デプロイ）で埋まっていた。
**枠を空けるのは Vercel の Deployment Retention 設定で、コードではない。**
コード側では1デプロイを小さくした。

- `scripts/prune-export.ts` を追加し `npm run build` の後段に入れた。
  Next.js 16 がルートごとに吐くセグメントプリフェッチ7ファイルのうち
  `__next._full.txt` は隣の `<route>.txt` の完全な複製で、`cacheComponents` を
  使っていないこのサイトでは誰も読まない（ブラウザの通信で要求0件を確認）。
  **198MB → 163MB、10,126 → 9,005ファイル。**
- `/papers` 系リンクを `prefetch={false}` に。ヘッダーの「研究カタログ」を
  Next が先読みするため、どのページでも `/papers` の3.6MBが余計に落ちていた。
  **未圧縮・キャッシュ空でトップ4.51→0.89MB、論文詳細4.49→0.87MB、
  `/papers` 8.13→4.64MB。**

**検証方法**（同じことを測り直すとき）: `npm run build` の後、`out/` を
クリーンURL対応の静的サーバで配って Chromium で開き、`request.sizes()` を
合計する。ファイルサイズを見るだけでは先読みぶんが見えない。

### 診療分野の軸と関連研究の作り直し（PR #38–#47 / 2026-08）

**きっかけ**: コードレビューの依頼から始まり、指摘13件の修正 → 分類軸の作り直し →
関連研究の再設計 → 収集の取りこぼし修正、と芋づる式に広がった。

**軸を `openalex_subfield` から独自辞書へ**（#39, #40）
OpenAlex の topic→subfield 写像が誤っていた（`Gastric Cancer Management` の親が
`Pulmonary and Respiratory Medicine`）。subfield「呼吸器」82件の半分が胃癌・前立腺癌
・大動脈だった。単一ラベルしか持てず「乳がん患者の眼有害事象＝乳腺＋眼科」も表せない。

`data/topic-areas.json`（25分野／全4,516トピック）を自前で持ち、**トピックIDで引く**
（名前で引くと OpenAlex の改名で写像が黙って外れる）。辞書に無いIDは
`data/unknown-topics.json` に溜まり `/status` に出る。分野の判断は人がやる。

**関連研究のスコアを組み直した**（#41, #42）
MeSHの重なりを独立した参照にしてAUCを測ると、加点していた4項目のうち3つが 0.5 未満
＝話題の一致に対して逆相関だった（研究カテゴリ0.456／使用DB0.443／解析手法0.439）。

ただしこの測定には落とし穴があり、**JADER論文の96%が
`Adverse Drug Reaction Reporting Systems` を共有する**ため、DB由来のMeSHを除かないと
結論が反転してDBに大きな重みを置くのが最善に見える。人が誤りと呼んだ挙動を、汚染された
指標がちょうど高く評価する。

結局、重み付けでは解けないと判断した（2案ともブラインド評価で有意差なし）。
順序は単一のスコアに任せ、**絞り込みをUIとして開いた**。「同じDBの研究が並ぶ」ことは、
DPCで他にどんな研究があるかを見たい人にとっては目的そのもので、1つの重み付けで全員を
満足させることはできない。

**MeSHを廃止**（#44）。収集も表示もやめた。索引付けが遅れるので収集時点では半数にしか
付かず、取り直す処理も無いので永久に埋まらない。辞書づくりとスコア検証に使い切った。

**収集の取りこぼしを直した**（#46, #47）
- 検索窓 14→90日。`hasabstract` で絞るため、アブストラクトが後から付いた論文は
  窓が狭いと二度と拾えない（実測で新規15→24件）
- 検索結果の100件上限を撤廃。PubMedは新しい順に返すので古い側を静かに捨てていた
- 代わりに取り込み数を50件に制限（LLMの作業量はこちらが決める）
- 偽陽性を `excluded-pmids.json` に記録（しないと毎週拾い直す）
- 週次同期は PR を作らず main へ直接push（**行き先の明示が必要**。上の「運用のハマりどころ」3を参照）
- 検証エラー時に該当論文だけを直す／直らなければ保留する手順を追加

**教訓**
- **代理指標は3回続けて「改善する」と予測し、3回ともブラインド評価で確認できなかった。**
  一方、その指標の汚染は人手の評価が独立に見つけた。どちらか一方では足りない。
- **条件付きの決定は実行を落としやすい。** 「辞書に使い切ってからMeSHを廃止」と決めて
  いたのに、辞書完成後に廃止を忘れていた（ユーザーの指摘で気づいた）。条件が満たされた
  時点で思い出す仕組みが要る。
- **クライアントコンポーネントから `lib/*.ts` を import すると `data/*.json` を
  引きずり込む。** 定数1つのために papers.json 全体4.1MBがバンドルされていた。
...
- クラウドセッションに `gh` CLI は無い → 組み込みGitHubツールを使う。
3. **main へは PR を作ってマージする。直接 push はできない**（2026-08-31 に判明）。
   セッションには「指定ブランチ以外へ無断で push しない」という規則が
   **システムプロンプトに注入されている**（リポジトリのどこにも書かれておらず、
   指示文では上書きできない）。指示文を「PRを作らず main へ直接push」に変えたところ、
   Routine は矛盾を検知して安全側に倒れ、自分のブランチに push して停止した。
   収集・分類は正常に終わっているのに本番へ反映されず、**Routineの表示は「成功」**。
   PR の作成とマージは「別ブランチへの push」ではなく API 操作なので、この規則と
   衝突しない。#22〜#37 が2ヶ月間無人で回っていたのはそのため。

**PRはレビューのためではなく、mainへ届くための経路である**（2026-09-01）
- 週次のPRは作成から5秒で自分でマージされ、誰もレビューしていない。そこだけ見ると
  無意味な儀式に見えるが、**ブランチ制限のあるエージェントが main に変更を届ける
  唯一の手段**である。2026-08-29 に「PRは作らない」へ変更したところ 8/31 の実行が
  止まり、2026-09-01 に PR方式へ戻した。
- 安全側の役割を担うのは PR ではなく、`validate-papers.ts`・`npm run build`・
  `/status` の可視化。PRを外しても安全性は下がらないが、**届かなくなる**。
