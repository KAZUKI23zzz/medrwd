...
| ~~MedlineCitation Statusで再取得制御~~ | 再取得ごと廃止（2026-08-29）。MeSHを使わなくなったため |
...
**残した課題**
- `/papers` の初回表示（当時 brotli後 728KB）: 全件の抄録を載せているため。検索精度を優先して現状維持。
  （その後、一覧で使わないフィールドを落として 705KB。抄録が生JSONの58%を占める点は変わらない）
...
- ~~MedlineCitation Status パース・保存~~（2026-08-29 廃止）
- ~~abstract空/MeSH未付与論文を月1回再取得、ステータスで終了判定~~（廃止済み）
- 収集頻度を毎日→毎週月曜に変更
