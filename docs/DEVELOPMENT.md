...
3. **push の行き先に `main` を明示する**（2026-08-31 に判明）。スケジュール実行の
   セッションは main ではなく自動生成ブランチにチェックアウトされるため、素の
   `git push` はそのブランチに行くだけで main に届かない。収集・分類が正常に
   終わっても本番に反映されず、しかもRoutineは「成功」と表示する。
   `push_files` に `branch: "main"` を渡すか、`git push origin HEAD:main` を使う。
   **push後に main の最新コミットを読み直して着地を確認すること**（手順8）。
- 2026-08 に PR方式（`claude/data-sync-*` を作って自分でマージ、#22〜#37）から
  main への直接pushへ切り替えた。**自分で5秒後にマージするPRは誰もレビューしておらず、
  防御になっていなかった**ため。安全側の役割は validate-papers と npm run build、
  そして `/status` の可視化が担う。
