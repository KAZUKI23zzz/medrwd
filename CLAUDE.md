...
| `data/false-positives.json` | 削除予定の偽陽性論文リスト（14件） |
...
- Claude Codeのサブエージェント方式で再分類中。詳細 → `docs/classification.md`
- **進捗: 445/903件完了（49.3%）、残り458件**
- 全件完了後に偽陽性削除（`data/false-positives.json`）→ フラグ名リネーム → コミット

**次（Track 2: 作業フロー見直し）**:
1. PubMed収集に `hasabstract` フィルター追加
2. Claude Routines で分類・翻訳を自動化
3. 反映 → Vercel自動リビルド
...
1. **再分類未完了**: 903件中445件済み。残り458件を数日に分散して処理
