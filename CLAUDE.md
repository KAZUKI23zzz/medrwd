...
| `data/papers.json` | 論文メタデータ（903件、445件分類済み） |
...
- Claude Codeの会話モデル（サブエージェント方式）で全903件を再分類中
- **進捗: 445/903件完了（49.3%）、残り458件**
...
### 仕組み
- 分類済みの論文には `haiku_classified: true` フラグが付く（※フラグ名は歴史的経緯。全件完了後にリネーム予定）
- 未分類論文は `papers.filter(p => !p.haiku_classified)` で取得できる

### 手順
1. **featureブランチを作成**（mainから）
2. **サブエージェント（Agent tool）を起動**し、以下を指示：
   - `data/papers.json` から未分類20件を抽出
   - 上記「分類スキーマ」セクションに従って分類
   - `/tmp/medrwd-batch.json` に書き出し → `data/papers.json` にマージ
3. サブエージェント完了後、次のバッチを起動（トークン分散のため間隔を空ける）
4. **全件完了後**: 偽陽性14件を削除 → `haiku_classified` を `classified` にリネーム → PRでmainにマージ
...
1. **再分類未完了**: 903件中445件が分類済み。残り458件をサブエージェント方式で処理中（数日に分散）
